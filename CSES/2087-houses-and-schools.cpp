...
void solve(){
    ...
    vi dp(n), ndp(n);
    /* dp[i] = till i everything is sorted and we have planted a school at i */
    auto costl = [&](int l, int r){return (b[r] - (l ? b[l - 1] : 0)) - (a[r] - (l ?  a[l - 1] : 0)) * (l + 1);};
    auto costr = [&](int l, int r){return (a[r] - (l ? a[l - 1] : 0)) * (r + 1) - (b[r] - (l ? b[l - 1] : 0));};
    auto cost = [&](int l, int r){
        if (l == r) return 0LL;
        int m = (l + r) >> 1;
        return costl(l, m) + costr(m + 1, r);
    };
    function<void(int, int, int, int)> solve = [&](int l, int r, int optl, int optr){
        if (l > r) return;
        int m = (l + r) >> 1;
        ndp[m] = inf; int opt = -1;
        rep(i, optl, m + 1) if (dp[i] + cost(i, m) < ndp[m]) ndp[m] = dp[i] + cost(i, m), opt = i;
        solve(l, m - 1, optl, opt), solve(m + 1, r, opt, optr);
    };
    rep(i, 0, n) dp[i] = costr(0, i);
    rep(i, 1, k){
        solve(0, n - 1, 0, n - 1);
        dp.swap(ndp);
    }
    int ans = inf;
    rep(i, 0, n) amin(ans, dp[i] + costl(i, n - 1));
    cout << ans << endl;
}
int32_t main(){
#ifdef coderdhanraj
