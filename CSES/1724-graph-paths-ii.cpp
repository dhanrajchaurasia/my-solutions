...
vec<int> mul(vec<int> &a, vec<int> &b){
    int nx = size(a), ny = size(b), nz = size(b[0]);
    vec<int> ab(nx, vi(nz, inf));
    rep(i, 0, nx) rep(k, 0, ny) rep(j, 0, nz) amin(ab[i][j], a[i][k] + b[k][j]);
    return ab;
}
void solve(){
    int n, m, k; cin >> n >> m >> k;
    vec<int> ans(1, vi(n, inf)); ans[0][0] = 0;
    vec<int> dp(n, vi(n, inf));
    rep(i, 0, m){
        int u, v, w; cin >> u >> v >> w, --u, --v;
        amin(dp[u][v], w);
    }
    while(k){
        if (k & 1) ans = mul(ans, dp);
        dp = mul(dp, dp);
        k >>= 1;
    }
    cout << (ans[0][n - 1] == inf ? -1LL : ans[0][n - 1]) << endl;
}
int32_t main(){
#ifdef coderdhanraj
