...
#define JAI_SHREE_RAAM ios_base::sync_with_stdio(false), cin.tie(nullptr), cout.tie(nullptr), cout.precision(6), cout.setf(ios::fixed);
/* ------------------------------------------------------- SOLUTION TO THE PROBLEM ------------------------------------------------- */
void solve(){
    int k; cin >> k;
    vec<double> ans(8, vector<double>(8, 1));
    rep(s, 0, 64){
        vec<double> dp(8, vector<double>(8));
        dp[s / 8][s % 8] = 1;
        rep(i, 0, k){
            vec<double> ndp(8, vector<double>(8));
            rep(a, 0, 8){
                rep(b, 0, 8){
                    vii p;
                    rep(c, 0, 4){
                        int na = a + dx[c];
                        int nb = b + dy[c];
                        if (na < 0 or nb < 0 or na > 7 or nb > 7) continue;
                        p.pb({na, nb});
                    }
                    for (auto &[na, nb] : p) ndp[na][nb] += dp[a][b] / size(p);
                }
            }
            dp.swap(ndp);
        }
        rep(a, 0, 8) rep(b, 0, 8) ans[a][b] *= (1 - dp[a][b]);
    }
    double res = 0;
    rep(i, 0, 8) rep(j, 0, 8) res += ans[i][j];
    cout << res << endl;
}
int32_t main(){
#ifdef coderdhanraj
