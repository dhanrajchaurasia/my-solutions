...
void solve(){
    ...
    int ans = 0;
    rep(i, 0, n){
        rep(j, 0, m) while (a[j].back() < i) a[j].pop_back();
        rep(j, 0, m) b[j] = a[j].back() - i;
        stack<int> stk;
        vi l(m, 0), r(m, m - 1);
        rep(j, 0, m){
            while (!stk.empty() and b[stk.top()] >= b[j]) stk.pop();
            if (!stk.empty()) l[j] = stk.top() + 1;
            stk.push(j);
        }
        while (!stk.empty()) stk.pop();
        bck(j, m, 0){
            while (!stk.empty() and b[stk.top()] >= b[j]) stk.pop();
            if (!stk.empty()) r[j] = stk.top() - 1;
            stk.push(j);
        }
        rep(j, 0, m) amax(ans, b[j] * (r[j] - l[j] + 1));
    }
    cout << ans << endl;
}
int32_t main(){
