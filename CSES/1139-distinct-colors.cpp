...
pprint::PrettyPrinter _printer(cerr);
#define debug(...) _printer.compact(true).print('[', #__VA_ARGS__, "] = ", __VA_ARGS__)
#else
#define debug(...) void(45)
#endif
...
private: int n;
    const T O{};
    vector<T> Tree;
public: BIT(int _n) : n(_n){Tree.assign(n + 1, O);}
    void op(T &a, T b){a += b;}
...
void solve(){
    ...
    function<void(int, int)> dfs = [&](int u, int p){
        ...
        if (last.count(a[u])) seg.add(tin[last[a[u]]], -1);
        last[a[u]] = u;
        seg.add(tin[u], 1);
        ans[u] = seg.query(tin[u], tout[u]);
    };
    dfs(0, -1);
    cout << ans << endl;
}
