...
void solve(){
    int n; cin >> n;
    var<int, 3> a(n);
    rep(i, 0, n) cin >> a[i][0] >> a[i][1], a[i][2] = i;
    sort(all(a), [](auto &a, auto &b){return a[0] == b[0] ? a[1] > b[1] : a[0] < b[0];});
    indexed_set<pii> s;
    vec<int> ans(2, vi(n));
    rep(i, 0, n){
        ans[1][a[i][2]] = size(s) - s.order_of_key({a[i][1], -1});
        s.in({a[i][1], i});
    }
    s.clear();
    bck(i, n, 0){
        ans[0][a[i][2]] = s.order_of_key({a[i][1], n});
        s.in({a[i][1], i});
    }
