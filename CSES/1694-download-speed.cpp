...
private: const i64 flow_inf = 1e18;
    struct edge{
        int v, u;
        i64 cap, flow = 0;
        edge(int v, int u, i64 cap) : v(v), u(u), cap(cap){}
    };
    ...
    bool bfs(){
        while (!q.empty()){
            int u = q.front(); q.pop();
            for (auto &id : adj[u]){
                if (e[id].cap - e[id].flow < 1 or ~d[e[id].u]) continue;
                d[e[id].u] = d[u] + 1, q.push(e[id].u);
            }
        }
        return d[t] != -1;
    }
    i64 dfs(int v, i64 flow){
        if (!flow) return 0;
        if (v == t) return flow;
        for (; ptr[v] < size(adj[v]); ptr[v]++){
            int id = adj[v][ptr[v]], u = e[id].u;
            if (e[id].cap - e[id].flow < 1 or d[v] + 1 != d[u]) continue;
            i64 pushed = dfs(u, min(flow, e[id].cap - e[id].flow));
            if (pushed) return e[id].flow += pushed, e[id ^ 1].flow -= pushed, pushed;
        }
        return 0;
    }
public: Dinic(int n, int s, int t) : n(n), s(s), t(t){adj.resize(n), d.resize(n), ptr.resize(n);}
