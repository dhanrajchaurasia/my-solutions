...
template<typename T, int MOD>class FFT{
public: void fft(vector<complex<double>> &a) {
        int n = size(a), L = 31 - __builtin_clz(n);;
        static vector<complex<long double>> R(2, 1);
        static vector<complex<double>> rt(2, 1);
        for (static T k = 2; k < n; k *= 2) {
        ...
        for (T k = 1; k < n; k *= 2) for (T i = 0; i < n; i += 2 * k) rep(j, 0, k) {
            auto x = (double*)&rt[j + k]; // include-line
            auto y = (double*)&a[i + j + k];  // exclude-line
            complex<double> z(x[0] * y[0] - x[1] * y[1], x[0] * y[1] + x[1] * y[0]);  // exclude-line
            a[i + j + k] = a[i + j] - z;
            a[i + j] += z;
        }
    }
    vector<T> convMod(const vector<T> &a, const vector<T> &b) {
        if (a.empty() or b.empty()) return {};
        vector<T> res(size(a) + size(b) - 1);
        int B = 32 - __builtin_clz(size(res)), n = 1 << B, cut = (int)sqrtl(MOD);
        vector<complex<double>> L(n), R(n), outs(n), outl(n);
        rep(i, 0, size(a)) L[i] = complex<double>((T)a[i] / cut, (T)a[i] % cut);
        rep(i, 0, size(b)) R[i] = complex<double>((T)b[i] / cut, (T)b[i] % cut);
        fft(L), fft(R);
        rep(i, 0, n) {
            int j = -i & (n - 1);
            outl[j] = (L[i] + conj(L[j])) * R[i] / (double)(2 * n);
            outs[j] = (L[i] - conj(L[j])) * R[i] / (double)(2 * n) / (complex<double>)1i;
        }
        fft(outl), fft(outs);
        rep(i, 0, size(res)) {
            ...
            if (cv >= MOD) cv %= MOD;
            long long ans = av;
            if ((ans *= cut) >= MOD) ans %= MOD;
            if ((ans += bv) >= MOD) ans -= MOD;
            if ((ans *= cut) >= MOD) ans %= MOD;
            if ((ans += cv) >= MOD) ans -= MOD;
            res[i] = ans;
        }
        return res;
    }
...
void solve(){
    ...
    */
    int n = size(s);
    vi a(n + 1), b(n + 1);
    rep(i, 0, n) if (s[i] == '1') a[i + 1] = b[n - i] = 1;
    auto f = FFT<int, (int)inf>().convMod(a, b);
    rep(i, n + 2, 2 * n + 1) cout << f[i] << " ";
}
int32_t main(){
#ifdef coderdhanraj
