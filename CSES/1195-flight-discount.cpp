...
void solve(){
    ...
    set<array<int, 3>> q;
    vec<int> dist(n, vi(2, inf));
    dist[0][0] = dist[0][1] = 0;
    q.in({0, 0, 0});
    while (!q.empty()){
        auto [wt, k, u] = *(q.begin()); q.erase(q.begin());
        for (auto &[v, w] : adj[u]){
            if (dist[v][k] > wt + w){
                array<int, 3> p = {dist[v][k], k, v}; 
                if (q.find(p) != q.end()) q.erase(p);
                p[0] = wt + w, dist[v][k] = p[0];
                q.in(p);
            }
            if (!k and dist[v][1] > wt + (w >> 1)){
                array<int, 3> p = {dist[v][1], 1, v}; 
                if (q.find(p) != q.end()) q.erase(p);
                p[0] = wt + (w >> 1), dist[v][1] = p[0];
                q.in(p);
            }
        }
    }
