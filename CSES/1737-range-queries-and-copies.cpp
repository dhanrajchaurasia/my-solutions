...
public: _node *L, *R; T val;
    _node(_node<T> *L, _node<T> *R, int val) : L(L), R(R), val(val){}
    _node(T val) : L(nullptr), R(nullptr), val(val){}
    _node(){_node(0);}
};
using node = _node<int>;
template<class T> class PST : public node{
private: int n;
public: PST(int n) : n(n){} 
    T op(T a, T b){return a + b;}
    node* build(int s, int e){
        if (s >= e) return new node();
        int mid = (s + e) >> 1;
        return new node(build(s, mid), build(mid + 1, e), 0);
    }
    node* update(node* prev, int s, int e, int p, T u){
        if (p < s or e < p or s > e) return new node();
        if (s == e) return new node(u);
        int mid = (s + e) >> 1;
        if (p <= mid){
            node* L = update(prev->L, s, mid, p, u), *R = prev->R;
            return new node(L, R, op(L->val, R->val));
        }
        node* L = prev->L, *R = update(prev->R, mid + 1, e, p, u);
        return new node(L, R, op(L->val, R->val));
    }
    T query(node *p, int s, int e, int l, int r){
        if (l > e or r < s or s > e) return 0;
        if (l <= s and e <= r) return p->val;
        int mid = (s + e) >> 1;
        return op(query(p->L, s, mid, l, r), query(p->R, mid + 1, e, l, r));
    }
    node* update(node* pre, int p, T u){return update(pre, 1, n, p, u);}
    T query(node* p, int l, int r){return query(p, 1, n, l, r);}
};
void solve(){
    int n, q; cin >> n >> q;
    PST<int> pst(n);
    node *f = pst.build(1, n);
    rep(i, 0, n){
        int x; cin >> x;
        f = pst.update(f, i + 1, x);
    }
    vector<node*> seg(1, f);
    rep(i, 0, q){
        int t, x, y, z; cin >> t >> x, --x;
        if (t == 1){
            cin >> y >> z;
            seg[x] = pst.update(seg[x], y, z);
        }else if (t == 2){
            cin >> y >> z;
            cout << pst.query(seg[x], y, z) << endl;
        }else seg.pb(seg[x]);
    }
}
int32_t main(){
