...
void solve(){
    int x, y; cin >> y >> x, --x, --y;
    int vis[8][8]; MEM(vis, 0);
    int ans[8][8], deg[8][8]; MEM(deg, 0);
    rep(i, 0, 8) rep(j, 0, 8) rep(k, 0, 8){
        int nx = i + dx[k + 8], ny = j + dy[k + 8];
        if (nx < 0 or ny < 0 or nx >= 8 or ny >= 8) continue;
        deg[i][j]++;
    }
    auto mark = [&](int id, int x, int y, bool add){
        ans[x][y] = (add ? id : -1), vis[x][y] = (add ? true : false);
        rep(i, 0, 8){
            int nx = x + dx[i + 8], ny = y + dy[i + 8];
            if (nx < 0 or ny < 0 or nx >= 8 or ny >= 8 or vis[nx][ny]) continue;
            deg[nx][ny] += (add ? -1 : 1);
        }
    };
    function<bool(int, int, int)> dfs = [&](int id, int x, int y){
        mark(id, x, y, true);
        ...
        rep(i, 0, 8){
            int nx = x + dx[i + 8], ny = y + dy[i + 8];
            if (nx < 0 or ny < 0 or nx >= 8 or ny >= 8 or vis[nx][ny]) continue;
            opt.pb({deg[nx][ny], nx, ny});
        }
        sort(all(opt));
        for (auto &[_, nx, ny] : opt) if (dfs(id + 1, nx, ny)) return true;
        mark(id, x, y, false);
        return false;
    };
    assert(dfs(1, x, y));
    rep(i, 0, 8) rep(j, 0, 8) cout << ans[i][j] << " \n"[j == 7];
}
int32_t main(){
#ifdef coderdhanraj
