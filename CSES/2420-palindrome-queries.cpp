...
private: int n, N;
    ...
    const T O = T(0, 0);
public: ST(){}
    ST(int _n) : n(_n), N(4 << __lg(n - 1)){seg.assign(N + 2, O);}
    T op(T x, T y){return (x + y);}
    void _update(int x, int s, int e, int p, mint u, mint v){
        if (e < s or s > p or e < p) return;
        if (s == e) return void(seg[x] = T(u, v));
        int m = (s + e) >> 1;
        _update(x << 1, s, m, p, u, v), _update(x << 1 | 1, m + 1, e, p, u, v);
    ...
    void update(int p, mint u, mint v){_update(1, 1, n, p, u, v);}
};
struct node{
    mint hashf, hashr;
    node(){}
    node(mint u, mint v){hashf = u, hashr = v;}
    node operator+(node &a){node f = a; f.hashf += hashf, f.hashr += hashr; return f;}
};
void solve(){
    int n, q; cin >> n >> q;
    string s; cin >> s;
    ST<node> f(n);  
    vector<mint> pwr(n + 1), ipwr(n + 1); pwr[0] = 1;
    rep(i, 1, n + 1) pwr[i] = pwr[i - 1] * 31; ipwr[n] = pwr[n].inv();
    bck(i, n, 0) ipwr[i] = ipwr[i + 1] * 31;
    auto getf = [&](int l, int r){
        mint res = f.query(l, r).hashf * ipwr[l - 1];
        return res;
    };
    auto getr = [&](int l, int r){
        l = n - l + 1, r = n - r + 1;
        swap(l, r);
        mint res = f.query(l, r).hashr * ipwr[l - 1];
        return res;
    };
    auto upd = [&](int x){
        mint a = pwr[x] * (s[x - 1] - 'a' + 1);
        mint b = pwr[x] * (s[n - x] - 'a' + 1);
        f.update(x, a, b);
        x = n - x + 1;
        a = pwr[x] * (s[x - 1] - 'a' + 1);
        b = pwr[x] * (s[n - x] - 'a' + 1);
    ...
    rep(i, 0, q){
        ...
        if (t == 1){
            ...
            cin >> y;
            cout << (getf(x, y) == getr(x, y) ? "YES" : "NO") << endl;
        }
    }
}
int32_t main(){
