...
public: int n, g;
    ...
    int find(int a){
        while (e[a] >= 0) s.pb(a), a = e[a];
        while (!s.empty()){int p = s.back(); s.pop_back(), e[p] = a;}
        return a;
    }
...
namespace LCA{
    ...
    int lca(int a, int b){
        ...
        if (a == b) return res;
        bck(i, 20, 0) if (up[a][i] != up[b][i]){
            amax(res, id[a][i]), amax(res, id[b][i]);
            a = up[a][i], b = up[b][i];
        }
        amax(res, id[a][0]), amax(res, id[b][0]);
        return res;
    }
    void dfs(int u, int p){
        for (auto &[v, idx] : adj[u]) if (v != p){
            d[v] = d[u] + 1, up[v][0] = u, id[v][0] = idx;
            rep(i, 1, 20) if (up[v][i - 1] != -1){
                id[v][i] = max(id[v][i - 1], id[up[v][i - 1]][i - 1]);
...
void solve(){
    ...
    rep(i, 0, m){
        ...
        adj[v].pb({u, i + 1}), adj[u].pb({v, i + 1});
    }
    MEM(up, -1);
    rep(i, 0, n) if (dsu.find(i) == i) dfs(i, -1);
    rep(i, 0, q){
        int u, v; cin >> u >> v, --u, --v;
