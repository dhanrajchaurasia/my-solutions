...
const int MOD[4] = {998244353, 1000000007, 1018199999, 1027799999};
const int mod = MOD[1];
...
int mod_inv(int n, int m = MOD[1]) {return power(n, m - 2, m);}
int mod_add(int a, int b, int m = MOD[1]) {a = a % m; b = b % m; return (((a + b) % m) + m) % m;}
int mod_mul(int a, int b, int m = MOD[1]) {a = a % m; b = b % m; return (((a * b) % m) + m) % m;}
int mod_sub(int a, int b, int m = MOD[1]) {a = a % m; b = b % m; return (((a - b) % m) + m) % m;}
int mod_div(int a, int b, int m = MOD[1]) {a = a % m; b = b % m; return (mod_mul(a, mod_inv(b, m), m));}
