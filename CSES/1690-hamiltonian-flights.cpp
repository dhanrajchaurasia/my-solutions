...
static int dp[(1 << N) - 1][N];
static bool vis[(1 << N) - 1][N];
static vi adj[N];
static queue<pii> q;
void solve(){
    int n, m; cin >> n >> m;
    ...
    q.push({0, 1}); vis[1][0] = true, dp[1][0] = 1;
    while (!q.empty()){
        auto [u, mask] = q.front(); q.pop();
        for (auto &e : adj[u]){
            if ((mask >> e) & 1) continue;
            int nmask = (1 << e) ^ mask;
            dp[nmask][e] += dp[mask][u];
            if (dp[nmask][e] >= mod) dp[nmask][e] -= mod;
            if(!vis[nmask][e]) q.push({e, nmask}), vis[nmask][e] = true;
        }
    }
