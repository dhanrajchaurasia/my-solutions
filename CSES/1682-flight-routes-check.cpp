...
public: int N;
    vector<int> p, s, e, r;
    int comp, maxi;
    DSU(int n) : N(n), comp(n), maxi(1){p.assign(n, -1), s.assign(n, 1), e.assign(n, 0), r.assign(n, 0);}
    T find(T a){return p[a] == a or p[a] < 0 ? a : p[a] = find(p[a]);}
    bool merge(T a, T b){
        a = find(a), b = find(b);
        if (a == b) return ++e[a], false;
        if (r[a] < r[b]) swap(a, b);
        amax(maxi, s[a] + s[b]);
        return --comp, r[a] += (r[a] == r[b]), p[b] = a, s[a] += s[b], e[a] += 1 + e[b], true;
    }
    bool same(T a, T b){return find(a) == find(b);}
};
void solve(){
    int n, m; cin >> n >> m;
    vec<int> adj(n), radj(n);
    rep(i, 0, m){
        int u, v; cin >> u >> v, --u, --v;
    ...
    function<void(int, int)> dfs2 = [&](int u, int p){
        vis[u] = 1;
        if (!dsu.same(u, p)) ans = MP(u, p);
        for (auto &e : adj[u]) if (!vis[u]) dfs2(e, p);
    };
    rep(i, 0, n) if (!vis[i]) dfs2(i, i);
    if (~ans.ff){
