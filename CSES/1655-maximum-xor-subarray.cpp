...
void insert(int n, Trie *root){
    ...
    bck(i, 31, 0){
        if ((n >> i) & 1){
            if (!cur->r) cur->r = new Trie();
            cur->cr++, cur = cur->r;
        }
        else{
            if (!cur->l) cur->l = new Trie();
            cur->cl++, cur = cur->l;
        }
    }
}
void remove(int n, Trie *root){
    Trie* cur = root;
    bck(i, 31, 0){
        if ((n >> i) & 1) cur->cr--, cur = cur->r;
        else cur->cl--, cur = cur->l;
    }
}
int maxXor(int n, Trie *root){
    ...
    bck(i, 31, 0){
        if ((n >> i) & 1){
            if (cur->l and cur->cl) res |= (1LL << i), cur = cur->l;
            else{
                if (!cur->r or !cur->cr) break;
                cur = cur->r;
            }
        }
        else{
            if (cur->r and cur->cr) res |= (1LL << i), cur = cur->r;
            else{
                if (!cur->l or !cur->cl) break;
                cur = cur->l;
            }
        }
    }
