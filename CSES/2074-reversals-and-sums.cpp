...
namespace Treap{
    struct node{
        node *l, *r;
        int wt, id, sz, rev, val, sum;
        node(int i, int sum = 0) : wt(rand()), id(i), sz(1), rev(0), val(sum), sum(sum){}
    };
    int sz(node *t){return t ? t->sz : 0LL;}
    int sum(node *t){return t ? t->sum : 0LL;}
    void push(node *t){
    ...
    void split(node *t, node *&l, node *&r, int k){// l = [1....k], r = [k+1.......n]
        ...
        if (push(t); sz(t->l) < k) split(t->r, t->r, r, k - sz(t->l) - 1), l = t;
        else split(t->l, l, t->l, k), r = t;
        t->sz = sz(t->l) + sz(t->r) + 1, t->sum = sum(t->l) + sum(t->r) + t->val;
    }
    ...
    void rev(node *&t, int l, int r){
        node *t1, *t2, *t3;
        split(t, t1, t2, l - 1), split(t2, t2, t3, r - l + 1);
        t2->rev ^= 1, merge(t, t1, t2), merge(t, t, t3);
    }
    node *at(node *t, int i){ // 0th indexing
        if (!t) return t;
        return push(t), sz(t->l) == i ? t : (sz(t->l) < i ? at(t->r, i - sz(t->l) - 1) : at(t->l, i));
    }
    int psum(node *t, int k){
        if (!t or !k) return 0LL;
        return push(t), sz(t->l) < k ? psum(t->r, k - sz(t->l) - 1) + t->val + sum(t->l) : psum(t->l, k);
    }
    node *init(int n, vi &a){
        node *t = 0;
        rep(i, 0, n) merge(t, t, new node(i, a[i]));
        return t;
    }
    vi inorder(node *t){
        vi order;
        ...
        function<void(node*)> dfs = [&](node *u){
            if (!u) return;
            push(u), dfs(u->l), order.pb(u->id), dfs(u->r);
        };
        dfs(t);
        return order;
    }
...
void solve(){
    ...
    rep(i, 0, k){
        int t, l, r; cin >> t >> l >> r;
        if (t == 1) rev(treap, l, r);
        else cout << psum(treap, r) - psum(treap, l - 1) << endl;
    }
}
int32_t main(){
#ifdef coderdhanraj
