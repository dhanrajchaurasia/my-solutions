...
void solve(){
    ...
    cin >> a;
    queue<array<int, 2>> q;
    vec<int> d(n, vi(m, inf)), dd(n, vi(m, inf));
    vec<int> p(n, vi(m, inf));
    rep(i, 0, n) rep(j, 0, m) if (a[i][j] == 'M') q.push({i, j}), d[i][j] = 0;
    while (!q.empty()){
        auto [x, y] = q.fr; q.pop();
        rep(i, 0, 4){
            ...
            if (a[x_][y_] == '#') continue;
            if (d[x_][y_] <= d[x][y] + 1) continue;
            d[x_][y_] = d[x][y] + 1;
            q.push({x_, y_});
        }
    }
    rep(i, 0, n) rep(j, 0, m) if (a[i][j] == 'A') q.push({i, j}), p[i][j] = -1, dd[i][j] = 0;
    while (!q.empty()){
        auto [x, y] = q.fr; q.pop();
        rep(i, 0, 4){
            int x_ = x + dx[i], y_ = y + dy[i];
            if (x_ < 0 or y_ < 0 or x_ >= n or y_ >= m) continue;
            if (a[x_][y_] == '#') continue;
            if (dd[x_][y_] <= dd[x][y] + 1) continue;
            if (dd[x][y] + 1 >= d[x_][y_]) continue;
            dd[x_][y_] = dd[x][y] + 1;
            p[x_][y_] = x * m + y;
            q.push({x_, y_});
        }
