...
public: Dinic(int n, int s, int t) : n(n), s(s), t(t){adj.resize(n), d.resize(n), nxt.resize(n);}
    ...
    vector<pii> minCut(){
        vector<pii> cut;
        for (int u = 0; u < n; u++) for (auto &id : adj[u]) if (e[id].flow > 0 and d[e[id].v] == -1) cut.emplace_back(u, e[id].v);
        return cut;
    }
};
void solve(){
    ...
    rep(i, 0, m){
        int u, v; cin >> u >> v, --u, --v;
        d.add_edge(u, v, 1), d.add_edge(v, u, 1);
        ok.insert({u, v});
    }
    cout << d.flow() << endl;
    for (auto &[u, v] : d.minCut()) if (ok.count({u, v})) cout << u + 1 << " " << v + 1 << endl;
}
int32_t main(){
#ifdef coderdhanraj
