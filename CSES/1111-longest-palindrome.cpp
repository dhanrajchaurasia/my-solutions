...
template<class T> class Manacher{
public: int n;
    vector<T> p;
    Manacher(string &s){
        string t = "#";
        for (auto &e : s) t += e, t += '#';
        n = size(t), p.assign(n, 1);
        for (int i = 1, l = 1, r = 1; i < n; i++){
            p[i] = max(T(0), min(r - i, p[l + r - i]));
            while (i + p[i] < n and i - p[i] >= 0 and t[i + p[i]] == t[i - p[i]]) p[i]++;
            if (i + p[i] > r) l = i - p[i], r = i + p[i];
        }
    }
    int get(int c, bool odd){
        int pos = 2 * c + 1 + !odd;
        return p[pos] - 1;
    }
};
void solve(){
    ...
    Manacher<int> man(s);
    rep(i, 0, n){
        int x = man.get(i, true);
        if (x >= r - l + 1) l = i - x / 2, r = i + x / 2;
        x = man.get(i, false);
        if (x >= r - l + 1) l = i - x / 2 + 1, r = i + x / 2;
    }
