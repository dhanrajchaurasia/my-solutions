...
int modd(int x, int m = mod) {x %= m; return (x < 0 ? x + m : x);}
mt19937_64 rng(chrono::steady_clock::now().time_since_epoch().count());
uniform_int_distribution<uint64_t> distrib(0, ULLONG_MAX);
#define urand distrib(rng)
...
public:
    ...
    node query(int x, int l, int r, int L, int R){
        ...
        int mid = (l + r) / 2LL;
        return combine(query(2 * x, l, mid, L, R), query(2 * x + 1, mid + 1, r, L, R));
    }
...
void solve(){
    ...
    while(q--){
        ...
        if (t == 2){
            int x;
            cin >> x;
            cout << min(s.query(1, x).right + x, s.query(x, n).left - x) << endl;   
        }else{
            int x, y;
            cin >> x >> y;
            node p;
