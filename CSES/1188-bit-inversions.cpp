...
public: ST(){}
    ...
    void _update(int x, int s, int e, int p, int u){
        if (e < s or s > p or e < p) return;
        if (s == e) return void(seg[x] = T(u));
        int m = (s + e) >> 1;
        _update(x << 1, s, m, p, u), _update(x << 1 | 1, m + 1, e, p, u);
    ...
    void update(int p, int u){_update(1, 1, n, p, u);}
};
struct node{
    int sum, psum, ssum, msum;
    node(){sum = msum = psum = ssum = 0;}
    node(int u){sum = psum = ssum = msum = u;}
    node operator+(node &a){
        node f; f.sum = sum + a.sum;
        f.psum = max(psum, sum + a.psum);
        f.ssum = max(ssum + a.sum, a.ssum);
        f.msum = max({msum, a.msum, ssum + a.psum});
        return f;
    }
};
void solve(){
    ...
    ST<node> f0(n), f1(n);
    rep(i, 0, n){
        if (s[i] == '0') f0.update(i + 1, 1),  f1.update(i + 1, -mod);
        else f1.update(i + 1, 1),  f0.update(i + 1, -mod);
    }
    int q; cin >> q;
    rep(i, 0, q){
        int x; cin >> x, --x;
        if (s[x] == '1') f0.update(x + 1, 1),  f1.update(x + 1, -mod);
        else f1.update(x + 1, 1),  f0.update(x + 1, -mod);
        cout << max(f0.query(1, n).msum, f1.query(1, n).msum) << (i == q - 1 ? endl : ' ');
        s[x] = '1' + '0' - s[x];
    }
}
int32_t main(){
