...
void solve(){
    ...
    rep(i, 0, n) id[--a[i]] = i;
    int ans = 1;
    rep(i, 1, n) ans += (id[i - 1] > id[i]);
    rep(i, 0, m){
        int x, y; cin >> x >> y, --x, --y;
        int u = a[x], v = a[y];
        if (u and id[u - 1] < id[u] and id[u - 1] > id[v]) ++ans;
        if (v and id[v - 1] < id[v] and id[v - 1] > id[u]) ++ans;
        if (u and id[u - 1] > id[u] and id[u - 1] < id[v]) --ans;
        if (v and id[v - 1] > id[v] and id[v - 1] < id[u]) --ans;
        if (u + 1 < n and id[u + 1] > id[u] and id[u + 1] < id[v]) ++ans;
        if (v + 1 < n and id[v + 1] > id[v] and id[v + 1] < id[u]) ++ans;
        if (u + 1 < n and id[u + 1] < id[u] and id[u + 1] > id[v]) --ans;
        if (v + 1 < n and id[v + 1] < id[v] and id[v + 1] > id[u]) --ans;
        if (abs(u - v) == 1){
