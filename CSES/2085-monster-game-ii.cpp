...
struct Line {
    mutable i64 k, m, p;
    bool operator<(const Line& o) const { return k > o.k; } // change for min-max
...
struct CHT : multiset<Line, less<>> {
    ...
    bool isect(iterator x, iterator y) {
        ...
        if (x->k == y->k) x->p = x->m > y->m ? -inf : inf; // change for min-max
