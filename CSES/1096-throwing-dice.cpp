...
template<class T, int mod = (int)(1e9 + 7)>
class MatrixExpotential{
private:
    int N;
    vector<T> term;
    vector<vector<T>> res, idx;
public:
    MatrixExpotential(vector<T> &a){ 
        this->N = size(a);
        this->term = a; // starting N terms [0...N-1] terms
        res.resize(N, vector<T>(N));
        idx.resize(N, vector<T>(N));
        for (int i = 0; i < N; i++)
        {
            res[i][i] = 1;
            idx[N - 1][i] = 1;
    ...
    vector<vector<T>> multi(vector<vector<T>> &a, vector<vector<T>> &b){
        vector<vector<T>> mul(N, vector<T>(N));
        for (int i = 0; i < N; i++)
            for (int j = 0; j < N; j++)
                for (int k = 0; k < N; k++)
