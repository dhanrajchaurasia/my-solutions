...
private: int n;
    const T O = INT_MIN;
    vector<T> seg;
public: ST(int _n) : n(_n){seg.assign(2 * _n + 1, O);}
...
namespace HeavyLight{
    const int N = 2e5 + 5;
    vi adj[N];
    int head[N], heavy[N], par[N], dep[N], pos[N];
    int dfs(int u = 0){
        int sz = 1, msz = 0, csz;
        for (auto &v : adj[u]) if (v != par[u]){
            par[v] = u, dep[v] = dep[u] + 1;
            sz += (csz = dfs(v));
            if (csz > msz) msz = csz, heavy[u] = v;
        }
        return sz;
    }
    void decomp(int n, int u = 0){
        for (int cpos = 0; u < n; ++u) if (par[u] == -1 or heavy[par[u]] != u){
            for (int v = u; ~v; v = heavy[v]) head[v] = u, pos[v] = ++cpos;
        }
    }
    void init(int n){
        rep(i, 0, n){
            adj[i].clear();
            head[i] = pos[i] = dep[i] = 0;
            heavy[i] = par[i] = -1;
        }
    }
    int query(int u, int v, ST<int> &seg){
        int ans = 0;
        for (; head[u] != head[v]; u = par[head[u]]){
            if (dep[head[u]] < dep[head[v]]) swap(u, v);
            amax(ans, seg.query(pos[head[u]], pos[u]));
        }
        if (dep[u] > dep[v]) swap(u, v);
        amax(ans, seg.query(pos[u], pos[v]));
        return ans;
    }
};
...
void solve(){
    ...
    dfs(), decomp(n);
    rep(i, 0, n) seg.update(pos[i], a[i]);
    rep(i, 0, q){
        int t, x, y; cin >> t >> x >> y, --t;
        if (t) cout << query(--x, --y, seg) << ' ';
        else seg.update(pos[--x], y);
    }
}
int32_t main(){
#ifdef coderdhanraj
