...
private: int n, N;
    const T O{};
    vector<T> seg, lazy1, lazy2;
public: ST(){}
    ST(int _n) : n(_n), N(4 << __lg(n - 1)){seg.assign(N + 2, O), lazy1.assign(N + 2, 0), lazy2.assign(N + 2, 0);}
    T op(T x, T y){return (x + y);}
    void push(int x, int s, int e){
        if (!lazy1[x] and !lazy2[x]) return;
        seg[x] += (e - s + 1) * lazy1[x];
        seg[x] += (e - s + 1) * (e - s + 2) / 2 * lazy2[x];
        if (s != e){
            int m = (e + s) >> 1;
            lazy1[x << 1] += lazy1[x], lazy1[x << 1 | 1] += lazy1[x];
            lazy2[x << 1] += lazy2[x], lazy2[x << 1 | 1] += lazy2[x];
            lazy1[x << 1 | 1] += (m - s + 1) * lazy2[x];
        }
        lazy1[x] = lazy2[x] = 0;
    }
    void _update(int x, int s, int e, int l, int r, T u, T v){
        if (e < s or s > r or e < l) return;
        push(x, s, e);
        if (l <= s and e <= r) return lazy1[x] += u + v * (s - l), lazy2[x] += v, void();
        int m = (s + e) >> 1;
        _update(x << 1, s, m, l, r, u, v), _update(x << 1 | 1, m + 1, e, l, r, u, v);
    ...
    void add(int l, int r, T u, T v){_update(1, 1, n, l, r, u, v);}
};
void solve(){
    int n, k; cin >> n >> k;
    ST<int> f(n);
    ...
    rep(i, 0, k){
        int t, l, r; cin >> t >> l >> r;
        if (t & 1) f.add(l, r, 0, 1);
        else cout << f.query(l, r) << endl;
    }
}
int32_t main(){
#ifdef coderdhanraj
