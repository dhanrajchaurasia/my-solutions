...
namespace Centroid{
    ...
    int find_size(int u, int p = -1){
        ...
        for (auto &v : adj[u]) if (v != p) sz[u] += find_size(v, u);
        return sz[u];
    }
    int find_centroid(int u, int p, int s){
        for (auto &v : adj[u]) if (!vis[v] and v != p and sz[v] > s) return find_centroid(v, u, s);
        return u; 
    }
    int a[N], k, mx;
    i64 ans;
    void op(int u, int p, bool add, int d = 1){
        if (d > k) return;
        if (add) a[d]++;
        else ans += a[k - d];
        for (auto &v : adj[u]) if (!vis[v] and v != p) op(v, u, add, d + 1);
        amax(mx, d);
    }
    int decompose(int u = 0, int p = -1){
        find_size(u);
        int c = find_centroid(u, -1, sz[u] >> 1);
        vis[c] = true, mx = 0;
        for (auto &v : adj[c]) if (!vis[v]) op(v, c, 0), op(v, c, 1);
        rep(i, 1, mx + 1) a[i] = 0;
        for (auto &v : adj[c]) if (!vis[v]) decompose(v, c);
        return c;
    }
};
