...
int dp[20][11][2][2];
int recur(string &s, int i, int prev, bool zero, bool tight)
{
    if (i == size(s))
        return 1;
    if (prev != -1 and dp[i][prev][zero][tight] != -1)
        return dp[i][prev][zero][tight];
    int x = tight ? s[i] - '0' : 9;
    int &ans = dp[i][prev][zero][tight] = 0;
    rep(j, 0, x + 1)
    {
        if (j == prev and !zero)
            continue;
        ans += recur(s, i + 1, j, (zero & (j == 0)), tight & (j == x));
    }
    return ans;
}
void solve()
