...
void solve(){
    ...
    vector<mint> dp(n);
    vi vis(n), cost(n, inf), dp_min(n, inf), dp_max(n); cost[0] = dp_min[0] = 0, dp[0] = 1;
    set<pii> pq; pq.insert({0, 0});
    while (!pq.empty()){
        auto [d, u] = *pq.begin(); pq.erase(pq.begin());
        if (vis[u]) continue; else vis[u] = 1;
        for (auto &[e, w] : adj[u]){
            if (cost[e] > cost[u] + w){
                cost[e] = cost[u] + w, dp_min[e] = inf, dp[e] = dp_max[e] = 0;
                if (pq.count({cost[e], e})) pq.erase({cost[e], e});
                pq.insert({cost[e], e});
            }
            if (cost[e] == cost[u] + w){
                amin(dp_min[e], dp_min[u] + 1);
                amax(dp_max[e], dp_max[u] + 1);
                dp[e] += dp[u];
            }
        }
    }
    cout << cost[n - 1] << " " << dp[n - 1] << " " << dp_min[n - 1] << " " << dp_max[n - 1] << endl;
}
int32_t main(){
#ifdef coderdhanraj
