...
int dp[10000][1LL << 10], n, m;
void generateMask(int i, int cur_mask, int prev_mask, vi &new_masks)
{
    if (i == n)
    {
        new_masks.pb(cur_mask);
        return;
    }
    if (!((1LL << i) & prev_mask))
    {
        generateMask(i + 1, cur_mask | (1LL << i), prev_mask, new_masks);
        if (i < n - 1 and !((1LL << (i + 1)) & prev_mask))
            generateMask(i + 2, cur_mask, prev_mask, new_masks);
    }
    else
        generateMask(i + 1, cur_mask, prev_mask, new_masks);
}
int recur(int col, int mask)
{
    if (col == m + 1)
        return (mask == 0);
    if (dp[col][mask] != -1)
        return dp[col][mask];
    vi new_masks;
    generateMask(0, 0, mask, new_masks);
    int res = 0;
    for (auto &cur_mask : new_masks)
        res = mod_add(res, recur(col + 1, cur_mask), mod);
