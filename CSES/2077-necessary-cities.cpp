...
void solve(){
    ...
    function<vi(void)> Tarjan = [&](){
        vi low(n), tin(n), a(n);
        int t = 0; vii edges;
        function<int(int, int)> dfs = [&](int u, int p){
            tin[u] = low[u] = ++t; int cnt = 0;
            for (auto &v : adj[u]){
                if (v == p) continue;
                if (tin[v]) amin(low[u], tin[v]);
                else{
                    dfs(v, u), ++cnt;
                    if (low[v] > tin[u]) edges.pb({u + 1, v + 1});
                    if (low[v] >= tin[u]) a[u] = 1;
                    amin(low[u], low[v]);
                }
            }
            return cnt;
        };
        rep(i, 0, n) if (!tin[i]) a[i] = dfs(i, -1) > 1;
        return a;
    };
    auto ans = Tarjan();
    cout << SUM(ans) << endl;
