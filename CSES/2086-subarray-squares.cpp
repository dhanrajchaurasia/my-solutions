...
int pre[N], dp[N][N];
/*
    dp[i][j] = i segments have been made with end point as j
    optl = maximum optimal l
    optr = minimum optimal r
*/
void dnc(int i, int l, int r, int optl, int optr){
    if (l > r) return;
    int mid = (l + r) / 2;
    pii best = MP(inf, l); // value, index
    rep(j, optl, min(mid, optr + 1)){
        int sum = pre[mid] - pre[j];
        amin(best, MP(sum * sum + dp[i - 1][j], j));
    }
    dp[i][mid] = best.ff;
    int opt = best.ss;
