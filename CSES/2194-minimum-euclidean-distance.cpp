...
void solve(){
    ...
    */
    vii a(n); cin >> a;
    set<pii> s; sort(all(a));
    int ans = LLONG_MAX, j = 0;
    auto rev = [](pii &x){return pii(x.ss, x.ff);};
    rep(i, 0, n){
        int d = ceil(sqrtl(ans));
        while(j < n and a[i].ff - a[j].ff >= d) s.erase(rev(a[j++]));
        auto it1 = s.lb({a[i].ss - d, a[i].ff});
        auto it2 = s.lb({a[i].ss + d, a[i].ff});
        while(it1 != it2){
            int dx = a[i].ff - it1->ss, dy = a[i].ss - it1->ff;
            amin(ans, sqr(dx) + sqr(dy));
            it1++;
        }
        s.insert(rev(a[i]));
    }
