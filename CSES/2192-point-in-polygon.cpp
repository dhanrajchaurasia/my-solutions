...
namespace Geometry{
    using db = double; using point = complex<double>; using line = pair<point, point>;
    #define x() real()
    #define y() imag()
    istream& operator>>(istream &in, point &p){db x, y; return (in >> x >> y, p.real(x), p.imag(y), in);}
    db dot(const point &a, const point &b){return real(conj(a) * b);}
    db cross(const point &a, const point &b){return imag(conj(a) * b);}
    db dist(const point &a, const point &b){return abs(a - b);}
    db slope(const line &a){return tan(arg(a.ss - a.ff));}
    line Line(const point &a, const point &b){return line(a, b);}
    point reflect(const point &p, const line &a){return a.ff + conj((p - a.ff) / (a.ss - a.ff)) * (a.ss - a.ff);}
    point project(const point &p, const line &a){return a.ff + (a.ss - a.ff) * dot(p - a.ff, a.ss - a.ff) / norm(a.ss - a.ff);}
    bool comp(const point &a, const point &b){return a.x() == b.x() ? a.y() < b.y() : a.x() < b.x();}
    bool isBetween(db p, db l, db r){return min(l, r) <= p and p <= max(l, r);}
    bool isBetween(const point &p, const point &l, const point &r){return isBetween(p.x(), l.x(), r.x()) and isBetween(p.y(), l.y(), r.y());}
    point intersect(const line &a, const line &b){db d1 = cross(b.ff - a.ff, a.ss - a.ff), d2 = cross(b.ss - a.ff, a.ss - a.ff); return (d2 * b.ff + d1 * b.ss) / (d2 - d1);}
    int sgn(db x){return (x > 0) - (x < 0);}
    bool isIntersect(const line &a, const line &b){
        db c1 = cross(a.ff - b.ff, a.ss - b.ff), c2 = cross(a.ff - b.ss, a.ss - b.ss), c3 = cross(b.ff - a.ff, b.ss - a.ff), c4 = cross(b.ff - a.ss, b.ss - a.ss);
        return (sgn(c1) ^ sgn(c2) and sgn(c3) ^ sgn(c4)) or (!c1 and isBetween(b.ff, a.ff, a.ss)) or (!c2 and isBetween(b.ss, a.ff, a.ss)) or (!c3 and isBetween(a.ff, b.ff, b.ss)) or (!c4 and isBetween(a.ss, b.ff, b.ss));
    }
};
...
void solve(){
    ...
    rep(i, 0, m){
        ...
        rep(j, 0, n){
            int k = (j + 1) % n;
            cnt += (a[j].x() <= x.x() and x.x() < a[k].x() and cross(a[j] - x, a[k] - x) < 0);
            cnt += (a[k].x() <= x.x() and x.x() < a[j].x() and cross(a[k] - x, a[j] - x) < 0);
            on += isBetween(x, a[j], a[k]) and cross(a[j] - x, a[k] - x) == 0;
        }
        if (on){
            cout << "BOUNDARY" << endl;
            continue;
