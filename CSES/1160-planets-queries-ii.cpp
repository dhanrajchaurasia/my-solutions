...
void solve(){
    ...
    int lg = __lg(n) + 1;
    vec<int> nxt(n, vi(lg));
    rep(i, 0, n) cin >> nxt[i][0], --nxt[i][0];
    rep(j, 1, lg) rep(i, 0, n) nxt[i][j] = nxt[nxt[i][j - 1]][j - 1];
    vi vis(n), len(n);
    ...
    function<int(int, int)> lift = [&](int u, int d){
        if (d <= 0) return u;
        rep(i, 0, lg) if ((d >> i) & 1) u = nxt[u][i];
        return u;
    };
    rep(i, 0, n) if (!vis[i]) dfs(i);
    while(q--){
        int u, v; cin >> u >> v, --u, --v;
        int p = lift(u, len[u]); // end of the cycle
        if (lift(u, len[u] - len[v]) == v){
            cout << len[u] - len[v] << endl;
            continue;
        }
        if (lift(p, len[p] - len[v]) == v){
            cout << len[p] + len[u] - len[v] << endl;
            continue;
        }
        cout << -1 << endl;
    }
