...
template <class T, const int MOD = (int)(1e9 + 7)>
class combinatorics{
private:
    vector<T> Fact, iFact;
public:
    ...
    int nPr(int n, int m){
        return (m < 0 or n < m ? 0 : (fact(n) * ifact(n - m)) % MOD);
    }
    int nCr(int n, int m){
        return (m < 0 or n < m ? 0 : (nPr(n, m) * ifact(m)) % MOD);
    }
    int Cn(int n){ // catalan number
        return (nCr(2 * n, n) * inv(n + 1, MOD)) % MOD;
    }
};
const int M = 1e6 + 10;
combinatorics<int> f(M);
