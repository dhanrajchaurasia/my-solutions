...
public: BinaryLift(){}
    BinaryLift(int _n, int root, vector<vector<int>> &adj) : n(_n){
        d.assign(n, 0), up.assign(n, vector<int>(MAX, -1));
        dfs(root, -1, adj);
    }
    void dfs(int u, int p, vector<vector<int>> &adj){
        for (auto &e : adj[u]){
            if (e == p) continue;
            d[e] = d[u] + 1, up[e][0] = u;
            for (int i = 1; i < MAX; i++) if (up[e][i - 1] != -1) up[e][i] = up[up[e][i - 1]][i - 1];
            dfs(e, u, adj);
        }
    }
    int kth_par(int a, int k){for (int i = 0; i < MAX; i++) if ((k >> i) & 1) a = up[a][i]; return a;}
    int lca(int a, int b){
        if (d[a] < d[b]) swap(a, b);
        a = kth_par(a, d[a] - d[b]);
        if (a == b) return a;
        for (int i = MAX - 1; i >= 0; i--) if (up[a][i] != up[b][i]) a = up[a][i], b = up[b][i];
        return up[a][0];
    };
...
void solve(){
    ...
    rep(i, 0, m){
        ...
        int p = f.lca(u, v);
        ++ans[u], ++ans[v], --ans[p];
        if (f.kth_par(p, 1) != -1) --ans[f.kth_par(p, 1)];
    }
    dfs(0, -1);
