...
public: int n, g;
    ...
    var<int, 5> history;
    DSU(){}
    DSU(int _n) : n(_n), g(n){e.assign(_n, -1);}
    void init(int _n){n = g = _n, e.assign(_n, -1);}
    int find(int a){
        while (e[a] >= 0) a = e[a];
        return a;
    }
    int sz(int a){return -e[find(a)];}
    bool merge(int a, int b){
        a = find(a), b = find(b);
        history.pb({a, b, e[a], e[b], g});
        if (a == b) return false;
        if (e[a] > e[b]) swap(a, b);
        return --g, e[a] += e[b], e[b] = a, true;
    }
    void rollback(){
        auto [u, v, eu, ev, gg] = history.back();
        e[u] = eu, e[v] = ev, g = gg;
        history.pop_back();
    }
};
template <class T, class U, class V> class DC{
private: int N;
    vector<U> res; V heap;
    vector<vector<T>> seg;
public: DC(int n, int m) : N(n){
        seg.resize(2 * N + 2), res.assign(N + 1, U());
        heap.init(m);
    }
    void add(int l, int r, T u){
    ...
    void run(int x = 1){
        for (auto &[u, v] : seg[x]) heap.merge(u, v);
        if (x >= N) res[x - N] = heap.g;
        else run(x << 1), run(x << 1 | 1);
        for (auto &u : seg[x]) heap.rollback();
    }
    U ans(int x){return res[x];}
};
void solve(){
    int n, m, k; cin >> n >> m >> k;
    DC<pii, int, DSU> d(k + 2, n);
    map<pii, int> last;
    rep(i, 0, m){
        int u, v; cin >> u >> v, --u, --v;
        if (u > v) swap(u, v);
    ...
    rep(i, 1, k + 1){
        ...
        if (u > v) swap(u, v);
        auto x = MP(u, v);
        if (t) d.add(last[x], i, x), last.erase(x);
        else last[x] = i + 1;
    }
    for (auto &[x, y] : last) d.add(y, k + 2, x);
    d.run();
    rep(i, 1, k + 2) cout << d.ans(i) << ' ';
}
int32_t main(){
#ifdef coderdhanraj
