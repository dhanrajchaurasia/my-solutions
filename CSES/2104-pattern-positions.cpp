...
template<typename T, typename Q> vector<T> SA(const Q &s){
    int n = size(s); vector<T> sa(n); iota(all(sa), T(0));
    stable_sort(all(sa), [&](int a, int b){return s[a] == s[b] ? a > b : s[a] < s[b];});
    vector<T> cls(n); for (int i = 0; i < n; ++i) cls[i] = s[i];
    for (int len = 1; len < n; len <<= 1){
        auto c = cls; for (int i = 0; i < n; i++){
            int j = sa[i - 1];
            cls[sa[i]] = (i and c[j] == c[sa[i]] and j + len < n and c[j + len / 2] == c[sa[i] + len / 2] ? cls[j] : i);
        }
        vector<int> cnt(n); auto ss = sa; iota(all(cnt), T(0));
        for (int i = 0; i < n; i++) ss[i] - len >= 0 ? sa[cnt[cls[ss[i] - len]]++] = ss[i] - len : T();
    }
    return sa;
}
template<typename T, typename Q> vector<T> LCP(const Q &s){
    int n = size(s); auto p = SA<T, Q>(s);
    vector<int> rank(n, 0), lcp(n, 0);
    for (int i = 0; i < n; i++) rank[p[i]] = i;
    for (int i = 0, k = 0; i < n; i++) if (rank[i] < n - 1){
        for (int j = p[rank[i] + 1]; s[i + k] == s[j + k]; ++k);
        lcp[rank[i]] = k ? k-- : k;
    }
    return lcp;
}
template <class T> class SP{
private: int N, K; const T O = T(INT_MAX); vector<vector<T>> t;
...
void solve(){
    string s; cin >> s;
    int n = size(s), q; cin >> q;
    auto sfx = SA<int, string>(s); // sorted indices based on suffices
    SP<int> sp(sfx);
    s += "$" + s; // appending dummy for comparisons
    rep(i, 0, q){
        string a; cin >> a;
        int lo = 0, hi = n - 1, m = size(a), l = -1, r = -1;
        while(lo <= hi){
            int mid = (lo + hi) >> 1;
            if (s.substr(sfx[mid], m) <= a) r = mid, lo = mid + 1;
            else hi = mid - 1;
        }
        lo = 0, hi = n - 1;
        while(lo <= hi){
            int mid = (lo + hi) >> 1;
            if (s.substr(sfx[mid], m) >= a) l = mid, hi = mid - 1;
            else lo = mid + 1;
        }
        if (~l and ~r and s.substr(sfx[r], m) == a){
            cout << sp.query(l + 1, r + 1) + 1 << endl;
            continue;
        }
        cout << -1 << endl;
    }
