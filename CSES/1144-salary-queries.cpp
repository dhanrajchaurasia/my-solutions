...
private: const T O{};
public: T op(T a, T b){return (a + b);}
    void add(int idx, T delta){for(; idx <= N; idx += (idx & -idx)) Tree[idx] = op(Tree[idx], delta);}
    T query(int idx){T res = 0; for(; idx > 0; idx -= (idx & -idx)) res = op(res, Tree[idx]); return res;}
    T query(int l, int r){return (l > r ? O : query(r) - query(l - 1));}
};
void solve(){
    int n, q; cin >> n >> q;
    vi a(n); cin >> a;
    BIT<int> f;
    rep(i, 0, n) f.add(a[i], 1);
    rep(i, 0, q){
        char t; cin >> t;
        int x, y; cin >> x >> y;
        if (t == '?') cout << f.query(x, y) << endl;
        else{
            --x;
            f.add(a[x], -1);
            a[x] = y;
