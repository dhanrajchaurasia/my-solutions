...
public: Dinic(int n, int s, int t) : n(n), s(s), t(t){adj.resize(n), d.resize(n), nxt.resize(n);}
    ...
    vector<pair<int, int>> minCut(vector<int> &l){
        vector<pair<int, int>> cut;
        for (auto &u : l) for (auto &id : adj[u]) if (e[id].flow == 1) cut.emplace_back(u, e[id].v);
        return cut;
    }
};
void solve(){
    ...
    int n = l + r;
    Dinic d(n + 2, 0, n + 1);
    rep(i, 1, l + 1) d.add_edge(0, i, 1);
    rep(i, 1, r + 1) d.add_edge(i + l, n + 1, 1);
    rep(i, 0, m){
        int x, y; cin >> x >> y;
        d.add_edge(x, y + l, 1);
    }
    cout << d.flow() << endl;
    vi left(l); iota(all(left), 1LL);
    for (auto &[u, v] : d.minCut(left)) cout << u << " " << v - l << endl;
}
int32_t main(){
#ifdef coderdhanraj
