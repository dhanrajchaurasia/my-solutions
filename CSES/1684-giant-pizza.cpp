...
private: int n;
    vec<int> adj, radj;
public: TwoSat(int n) : n(n){adj.assign(2 * n, {}), radj.assign(2 * n, {});}
    int norm(int u){
        bool neg = u < 0;
        u *= (neg ? -1 : 1);
         --u, u <<= 1, u ^= neg;
    ...
    void add(int u, int v){
        u = norm(u), v = norm(v);
        adj[u ^ 1].pb(v), adj[v ^ 1].pb(u);
        radj[v].pb(u ^ 1), radj[u].pb(v ^ 1);
    }
    vi solve(){
        vi vis(2 * n), comp(2 * n), order;
        ...
        reverse(all(order));
        vis.assign(2 * n, 0);
        vi p = order; int col = 0;
        rep(i, 0, 2 * n) swap(adj[i], radj[i]);
        for (auto &u : p) if (!vis[u]){
            order.clear(), ++col, dfs(u);
            for (auto &v : order) comp[v] = col;
        }
        vi ans(n);
        rep(i, 0, n){
            if (comp[i << 1] == comp[i << 1 | 1]) return {};
            ans[i] = comp[i << 1] > comp[i << 1 | 1];
        }
        return ans;
    }
}; // Imp: add() give one or both true, use add() to make ur boolean exp by bitAnd
void solve(){
    int n, m; cin >> n >> m;
    TwoSat t(m);
    rep(i, 0, n){
